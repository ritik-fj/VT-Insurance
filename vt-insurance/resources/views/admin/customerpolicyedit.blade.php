...
@section('content')
    ...
    <div class="container p-5 my-3 border">
        ...
        <form action="{{ route('customer_policy.update', $customerPolicy->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label class="form-label"> <strong> Policy Type</strong></label>
                <input type="text" class="form-control" value="{{ $customerPolicy->policy_type }}" name="policy_type"
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="coverage_amount" name="coverage_amount"
                    placeholder="Enter Coverage Amount" value="{{ $customerPolicy->coverage_amount }}">
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="premium_amount" name="premium_amount"
                    placeholder="Enter Policy Amount" value="{{ $customerPolicy->premium_amount }}">
            </div>
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="excess_amount" name="excess_amount"
                    placeholder="Enter Excess Amount" value="{{ $customerPolicy->excess_amount }}">
            </div>
            <div class="mb-3">
                <label class="form-label"> <strong> Policy Duration</strong></label>
                <input type="text" value="{{ $customerPolicy->policy_duration }}" class="form-control"
                    name="policy_duration" placeholder="Enter Policy Duration">
            </div>
            <div class="mb-3">
                <label class="form-label"> <strong>Description</strong></label>
                <textarea id="description" name="description" class="form-control" rows="3" required>{{ $customerPolicy->description }}</textarea>
            </div>
